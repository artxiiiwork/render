  ...
  if (text.startsWith("/start")) {
    ...
    if (!code) {
      await sendTelegramMessage(
        chatId,
        "Привет! Это бот уведомлений RENDER. Чтобы подключить уведомления, зайдите в личный кабинет на сайте → блок «Уведомления в Telegram» → «Подключить»."
      );
      return NextResponse.json({ ok: true });
    }
    ...
    try {
      ...
      await sendTelegramMessage(
        chatId,
        "✅ Готово! Уведомления RENDER подключены. Будем присылать сюда новые сообщения и отклики."
      );
    } catch (e) {
      if (
